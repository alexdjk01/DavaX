class FactTable:
    ...
    def populate_fact_timesheets(self):
        project_id = self.get_project_id(self.cursor, "DavaX")
        ...
        for event_id, event_name, event_type, event_date in self.cursor.fetchall():
            date_id = self.get_date_id(self.cursor, event_date)
            if date_id is None:
                continue
            for emp_id, qty in self.get_event_quantities(self.cursor,
                                                         event_name,
                                                         event_type,
                                                         event_date):
                self.insert_fact(self.cursor,
                                 emp_id,
                                 event_id,
                                 date_id,
                                 project_id,
