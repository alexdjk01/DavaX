...
first_attendance = load_attendance("./files/etl_meeting_1.csv")
second_attendance = load_attendance("./files/etl_meeting_2.csv")

# --- Step 3: Combine both attendance meetings and drop duplicates (achieve employee only once)
attendees = pd.concat([first_attendance, second_attendance]).drop_duplicates(subset="Email")  # drop on duplicate email
...
final_df.to_csv("./files/source_employees.csv", index=False)
